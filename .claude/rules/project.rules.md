...
- **同梱スクリプトは Node の組み込みモジュールだけで書く**: プラグインは配布先へコピーされるだけでインストール工程が無く、リポジトリに `package.json` も置かない。`skills/<name>/scripts/**` の `.mjs` が npm パッケージを import すると、配布先では解決できず起動時に失敗する。import は `node:` 接頭辞付きの組み込みモジュールに限る。ブラウザへ読ませるライブラリはこの制約の対象外で、`public/*.html` からバージョンを固定した CDN で読み込む
