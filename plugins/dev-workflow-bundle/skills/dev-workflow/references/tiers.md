...
At the end of Create Plan, assess the drafted plan against § Tier criteria once more. The tier only rises: an equal or lower result changes nothing. When it rises, emit one line naming the old and new tier, then set every phase row the new tier runs back from `completed` (skipped) to `pending`, per `SKILL.md` § Difficulty and the skip table. Plan Review has not run yet, so nothing is re-done.
