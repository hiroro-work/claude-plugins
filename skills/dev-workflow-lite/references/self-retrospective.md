...
Read from `SKILL.md` Phase 17 (Self-Retrospective). Unqualified `§` references point into this file. The phase turns this run's friction into Findings about the skills that caused it, and posts them where the skill maintainers triage them. It is built not to grow the skills: every Finding must say what behavior changes and what it costs in characters, and a Finding that would only add prose is not emitted.
...
4. Post. Repo mode: `Write` the body to `.claude/plans/<slug>.retrospective.md`, then `gh api --method POST /repos/<feedback>/issues -f title="<title>" -F body=@.claude/plans/<slug>.retrospective.md`; delete the file on success. Path mode: `Write` `<dir>/<YYYY-MM-DD>-<slug>.md`. A failure is reported with the body left in chat and the file left on disk; no retry.
