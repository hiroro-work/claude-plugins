...
- プラグイン名: kebab-case（例: `plugin-security`, `ask-claude`）
- スキル名: kebab-case（例: `plugin-security`, `ask-peer`）
...
プラグイン追加時は `/plugin-security --project` でセキュリティスキャンを実行し、問題がないことを確認してください。
