...
- **Why the drawing lives in `artifactor`.** Drawing the page was the slow part of a turn and knowing how to draw it was the heavy part of a context, so both moved to a background agent that never touches the chat; nothing in how that agent is run — one agent per session, a message per turn, publishing when it reports back, waiting only at the wrap-up — depends on what kabeuchi puts on the page. That part is now its own skill, `artifactor`, which any conversation can use to grow a page. Kabeuchi keeps what is its own: which cards a reader needs, in which register, and when the session wraps up. Orientation still publishes the first two cards before sending the rest, so the reader gets a URL early.
