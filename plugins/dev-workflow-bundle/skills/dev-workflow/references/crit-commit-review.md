...
## Round verification and fix accumulation

What a round owes once its `scope: "line"` edits have landed. § Procedure step 5's `scope: "line"` bullet is the only entry, reached once every line comment in the round has been routed and the ones routed to **apply** have been applied. A round that applied nothing — every comment queued — never reaches here.

Run **Step 7 (Check / Test)'s `check_commands` phase alone**, without its `test_commands` phase, once for the whole round. That Step 7 entry does **not** re-fire Step 7's concurrent rules-review / code-review launches. On a `check_commands` failure, follow Step 7's fail-stop discipline (retry, then report and stop); a fix applied to clear it is one of this round's edits and is accumulated below like any other.

Then **append every path this round's edits touched to `commit_fix_files`** (`SKILL.md` Step 1 sub-step 6's cross-step variable init table) and return to this round's comment processing. No user gate fires here.

**The test phase and the rules-compliance walk are not run per round.** Both belong to [`finish-phase.md`](finish-phase.md) § Step 10.5, which runs them once over the union of every round's edits after the last commit has landed.
