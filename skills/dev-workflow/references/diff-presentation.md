...
| --- | --- | --- | --- | --- |
| [`interactive-commits.md`](interactive-commits.md) — Step 10's per-commit candidate (steps 1 and 2, not 1.5, on its pathspec and refreshed-boundary derivations, not on its plain boundary one; step 3 on all three) | `HEAD`, via `git rev-parse HEAD` | once per commit, when that commit's candidate is built | only on its non-landing exits (closed list at its § Per-commit loop sub-step `a`, which also names the restore each derivation owes — step 5's unstage on the pathspec one, `git read-tree HEAD` on the refreshed one) | none — its objects land through a real `git commit`, where the project's own hooks fire |
...
| [`step5-implement.md`](step5-implement.md) — Step 5 per-Build-order-step boundary (steps 1–3 and 5 only; it records objects for Step 10 to consume rather than reviewing them) | `step5_boundary_base` | initialized to `<base-commit>` at Step 5 entry, then advanced to the previous step's `<head>` as each Build order step's edits land | once, at Step 5 exit | `boundary_check_commands`, on every run |
