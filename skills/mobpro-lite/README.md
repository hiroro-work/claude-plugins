...
A thin entry point: `/mobpro-lite <task>` runs `/dev-workflow-lite --mob <task>`. Everything about the run — phases, gates, settings, the junior-facing stops and narration — is defined by `dev-workflow-lite` and its `references/mob-mode.md`. Install both plugins.
