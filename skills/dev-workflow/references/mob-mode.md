...
3. **Detail checkpoints (USER GATE, once per run, before the plan is written)**. Do the research on the main thread and narrate what it concluded, never the path that reached it. Segment it by content, not by count: one checkpoint carries one thing the plan rests on — the code one decision turns on, one constraint that rules an approach out, or the proposed build order — and one that would introduce more than three files the junior has not yet met is two checkpoints. The count follows from the research; the junior's load is what one checkpoint carries, not how many there are. The first checkpoint opens its own turn, not the approach narration's. Each checkpoint opens with its purpose — what you looked at and why this task needs it, naming the part of the orientation draft it feeds when there is one — then says whether that code is on the orientation map or is reference code the task will not change (a similar feature to copy, a constraint to respect), then what it does today, and only then what follows for the plan. A checkpoint shows the code a decision turns on; the decision itself is never put to the junior here — it goes to `### Choices I made`, settled at § Plan Approval. The first one says how many parts are left. Every checkpoint ends with an open invitation as the turn's last line — in the resolved language, worded like "Is anything unclear so far?" rather than a demand to name gaps — then waits. Replies: **go on** (nothing open, or a go-ahead that also says the point did not land — note it for the Completion learning summary and advance without re-explaining) → next checkpoint or plan authoring; **question** → answer, ask again; **change** (look elsewhere, different direction) → do it, share, ask again; **not an answer** → ask a confirming question, neither advance nor re-share. A reply doubting the output arrived → re-share the checkpoint, shorter. If a checkpoint undercuts the approach narration, say so there and restate it.
