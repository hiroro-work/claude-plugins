...
3. **Removed keys (tombstone)**: `checkpoint` / `quiz` / `error_reading_practice` no longer exist. `Glob` `.claude/mobpro*.md` — one call covering both former layers, and not a `Read`, since their contents are never parsed. On any hit, emit **one** warning line naming every file found and stating that `mobpro` no longer reads them; presence is never an error.
