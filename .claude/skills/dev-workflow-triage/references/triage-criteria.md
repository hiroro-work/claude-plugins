...
- [ ] The problem the Finding describes **reproduces in the current file state** — read `<Edit root>/SKILL.md` and `<Edit root>/references/*.md` (per SKILL.md `§ Fixed configuration`'s target table; the `references/` half is vacuous for a target without that directory) and confirm the ambiguity / missing-branch / wrong-default / rules-conflict is still present.
- [ ] The `Suggested fix direction` translates to a **local edit** — a paragraph rewrite, a new clause inside an existing section, an explicit step addition, a rules-reference insertion. It should be expressible as one `Edit` tool call (one `old_string` → `new_string`). Sole exception: on a test-gated target (Size gate column names a test file) whose size test fails after the Edit, SKILL.md § 3.4 sub-step (a.5) applies the deletion the Finding's `Size delta` names after `pays for it:` as a second `Edit` — for that pair only.
- [ ] The edit target sits inside the target's canonical directory — the Edit root in SKILL.md `§ Fixed configuration`'s target table (SKILL.md or `references/*.md` under it). Never a rules file, never a different skill, never outside those directories.
...
2. **Out-of-scope target mentioned in description**: `Target skill` is within the triage scope (the targets in SKILL.md `§ Fixed configuration`'s target table), but the `Description` body actually complains about a different skill's behavior (in or out of scope). Not the triage target's fault to fix.
...
7. **Already addressed in a later producer version**: stale-issue path applied when the issue's Producer version predates the current `<producer-plugin>` version — `current_version[producer_name]`, the marketplace version of the plugin named on the Producer version line (or either side is `unknown`). See SKILL.md § 3.3 Judge each Finding § Version-aware judgment for the (i) CHANGELOG entry + (ii) SKILL.md cite gates, the either-leg doubt fall-through to standard checklist, and the reason-string format.
8. **Producer's own size rule violated**: the Finding carries `Fix kind: wording` together with a positive `Size delta` (the `+N` form with `N ≥ 1`; `+0` is allowed). The lite producer emits `wording` Findings only when the direction deletes or replaces text at equal or smaller size, so this combination contradicts the producer's own rule. Reject and cite both field values. Not applicable when either field is absent.
...
| `Fix kind` / `Size delta` present or absent, any value | Not a parse-error — optional-recognized fields (lite producer); captured when present, otherwise ignored |
| `Fix kind: wording` with a positive `Size delta` | Per-Finding `reject` (Reject #8 — violates the producer's own size rule) |
| `Target skill` outside the triage scope (the targets in SKILL.md `§ Fixed configuration`'s target table) | Whole issue → `parse-error`; post comment; leave open |
...
| `marketplace.json` has no entry for the producer's plugin (`dev-workflow` or `dev-workflow-lite`, or file missing) at producer or consumer time | Treat the resolved version as `unknown`. Producer emits `**Producer version:** <producer> vunknown`; consumer's `producer_version == "unknown"` (or `current_version[producer_name] == "unknown"`) triggers Reject #7 stale-issue path on every Finding (still gated by the (i)+(ii) AND with doubt fall-through, so false rejects remain bounded) |
...
| (a.5) Char-budget check: projected chars still exceed `cap[target]` after the one consolidation-form redraft attempt (or no redraft is admissible) | Per-Finding `conflict`; skip (b)–(g); record warning `skill-tree-growth budget exceeded (<projected> > cap <cap>) — deferred`; mark for `budget-deferred` label at § 3.5 |
| (a.5) Size-test gate (targets whose Size gate column names a test file — `dev-workflow-lite` / `mobpro-lite`): a `chars <=` size assertion of `node --test tests/dev-workflow-lite/skill-size.test.mjs` still fails after the Edit plus the `pays for it:` deletion Edit (or no deletion is named) | `git checkout HEAD -- <paths>` reverting both edits; `record.target` reset to `—`; per-Finding `conflict`; skip (c)–(g); record warning `size test failed (<assertion name>) — needs a deletion or a budget change`; **no** `budget-deferred` label; issue stays open; counted at Step 4 under `size-gate conflicts` |
| (a.5) Size-test gate: any other non-zero exit (`node` missing, a non-size assertion failing, wrong cwd) | `git checkout HEAD -- <path>`; `record.target` reset to `—`; ordinary per-Finding `conflict`; skip (c)–(g); record warning `size test could not be evaluated (<reason>)`; **no** `budget-deferred` label |
...
| Step 3.7 version-skew guard: `dev-workflow-bundle` is in the bump set AND the pre-bump `dev-workflow` version is strictly greater than `dev-workflow-bundle`'s (`bundle ≥ dev-workflow` passes; a lite-only run runs no check) | Abort release bookkeeping with no marketplace / CHANGELOG edits; record `release-bookkeeping=failed (version skew: dev-workflow=<v1>, dev-workflow-bundle=<v2>)` |
...
| `dev-workflow-triage`-targeted Finding accepted | Edit target is `.claude/skills/dev-workflow-triage/...`; commits normally in § 3.4; excluded from § 3.7 release bookkeeping by the (b) filter (Release bookkeeping `none (project-local)` — no version bump / CHANGELOG) |
| `dev-workflow-lite` / `mobpro-lite`-targeted Finding accepted | Edit target is under `skills/<target>/`; commits normally in § 3.4; no (f.5) bundle copy sync (Bundle member `no`); § 3.7 bumps that plugin alone (Release bookkeeping `alone`, no `dev-workflow-bundle` pairing) with a bare `### <skill> vX.Y.Z` CHANGELOG heading |
