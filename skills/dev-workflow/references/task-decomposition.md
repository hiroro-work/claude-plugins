...
When a state file is in play, surface parent progress with a single top-level progress row (created via `TaskCreate`): `Parent task: <done>/<total> subtasks done — <slug>`. This row is a progress **display**, not a work item — keep its status as `pending` throughout the parent task's lifetime. Never mark it `in_progress`. Refresh the `<done>/<total>` count (via `TaskUpdate`) whenever a subtask finishes, and delete the row entirely when the state file is deleted. The row is a Task-tools display only — a run tracking its phases in the progress file registers none. Only insert the row once a state file actually exists (i.e. after [`task-decomposition-resume.md`](task-decomposition-resume.md) § A step 5, or after [`task-decomposition-normal.md`](task-decomposition-normal.md) § B step 3.f). A run that stays undecomposed gets no row.
