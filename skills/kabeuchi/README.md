...
The reader can also be changed mid-session: say who you are in the chat, and the page is rewritten for that reader on the same turn. Cards that teach what a reader already knows — what the subject is, its pieces, how they work, their names, its words — are left out for that reader.
...
- The `artifactor` skill, which draws and publishes the page (the bundle carries it; install it alongside when kabeuchi is installed alone). Its requirements — `Agent`, `SendMessage` and the fallbacks when they are missing — are in its README.
...
- **Why the drawing lives in `artifactor`.** Drawing the page was the slow part of a turn and knowing how to draw it was the heavy part of a context, so both moved to a background agent that never touches the chat; nothing in how that agent is run depends on what kabeuchi puts on the page. That part is now its own skill, `artifactor`, which any conversation can use to grow a page. Kabeuchi keeps what is its own: which cards a reader needs, in which register, and when the session wraps up. Orientation still publishes the first two cards before sending the rest, so the reader gets a URL early.
...
- **Why The pieces is two cards.** One picture cannot be both followable and complete: a reader meeting every part at once, each with two or three sentences of label on it, has no place to start. The second card keeps the first's layout, so a reader who has learned the shape reads the detail without learning it again. Nothing is dropped; the sentences that came off the first picture are what the second card holds.
- **The slug rule is written out again in this skill's own `SKILL.md`**, because bundle members install independently and cannot locate a sibling's files at run time. When it changes in `dev-workflow`, align this skill by hand. The `language` resolution is not duplicated: this skill loads `artifactor` before resolving it and reads that skill's rule. The register started from `references/mob-mode.md` and is now this skill's own.
