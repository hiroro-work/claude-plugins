...
Setting `boundary_check_commands` there runs those commands as each implementation unit's object is built, so the diff the junior reviews is already the content that will be committed. `mobpro` runs them on every run — it builds a unit object regardless of `interactive_commits`, which in `dev-workflow` gates the equivalent step.
