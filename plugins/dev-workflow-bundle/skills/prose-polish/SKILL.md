...
- **File mode** — given one or more file paths, rewrite the target-language natural-language prose **in place** (comments, test descriptions, docstrings, user-facing strings). Code, identifiers, English technical terms, and logic-bearing string literals are left untouched.
...
>
> **Preserve everything that is not target-language prose (hard constraint)**: never change code, identifiers, function / variable / type names, English technical terms, import paths, or any string literal that carries program logic (keys, enum values, format specifiers, paths, commands). Rewrite **only** the natural-language prose written in the target language; leave prose written in other languages untouched. If a candidate change could alter program behavior or touch a non-prose token, do not emit it.
>
...
>
> **Preserve non-prose tokens (hard constraint)**: keep identifiers, code fragments, English technical terms, file paths, and command / config strings verbatim — refactor only the natural-language wording around them. Do not add, drop, or reorder factual content; only improve concision and naturalness. If the text is already concise and natural, return it unchanged.
