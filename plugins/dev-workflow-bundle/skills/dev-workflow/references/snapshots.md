...
- Temp index `.git/dev-workflow.index` (literal path). Snapshots are built there so the user's index is never touched.
...
3. **Fold** in a throwaway worktree:
   ...
   ```
   For each target in order: `git -C .git/dev-workflow-wt apply --unidiff-zero "<patch>"`, `git -C .git/dev-workflow-wt add -A` (allowed here only because the worktree is throwaway and holds nothing but the tip plus this patch), `git -C .git/dev-workflow-wt commit --no-verify --no-gpg-sign --fixup=<commit>`. If an apply fails, append that patch to the trailing set and continue. Then:
   ```bash
