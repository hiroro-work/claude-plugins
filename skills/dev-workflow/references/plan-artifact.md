...
2. Write the exchange that shaped this plan to `.claude/plans/<slug>.dialogue.md`, from what was said rather than from memory. Two sources together:
   - Chat turns: run `node "<base dir>/scripts/retro/session-text.mjs" --since <the time on the first line of this run's timing log>`. On a non-zero exit, on empty output, or on output carrying no `user:` line, say in one line which of those happened and use what is in context instead.
