...
- **opt-out signal の観測チャネル（closed list）**: (a) 本 repo の dogfooding config、(b) `<self-retrospective destination repo>`（self-retrospective 経由の実走行 signal）、(c) 本 marketplace repo の GitHub issues、(d) 既知利用プロジェクト（<known internal projects>）の config 確認。配布先の完全観測は不可能なので、「観測」から「告知 + 待機期間」への転換が設計思想
