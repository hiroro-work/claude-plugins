...
| localization-enumeration | `references/configuration.md`, the `language` bullet's enumeration of localized outputs (`SKILL.md` § Configuration's `language` bullet is now a one-line index into it) | `references/localization.md` § Localization granularity, opening "Applies to" sentence (`references/plan-format.md`'s same-named heading is now a delegating pointer to it) | Same output-category set (each category present on both sides) |
