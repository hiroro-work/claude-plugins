...
- `checked_count`: number of bundle member entries actually inspected (8 at the time of writing — `ask-peer`, `dev-workflow`, `extract-rules`, `rules-review`, `tidy`, `prose-polish`, `mobpro`, `kabeuchi`). If the list could not be loaded (`EXECUTION_ERROR` from Step 1), set this to `0`.
