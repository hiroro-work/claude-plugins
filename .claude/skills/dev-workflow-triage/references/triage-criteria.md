...
7. **Already addressed in a later producer version**: stale-issue path applied when the issue's Producer version predates `current_version` — the marketplace version of the `dev-workflow` producer plugin (or either side is `unknown`). See SKILL.md § 3.3 Judge each Finding § Version-aware judgment for the (i) CHANGELOG entry + (ii) SKILL.md cite gates, the either-leg doubt fall-through to standard checklist, and the reason-string format.
8. **Producer's own size rule violated**: the Finding carries `Fix kind: wording` together with a positive `Size delta` (the `+N` form with `N ≥ 1`; `+0` is allowed). The v2 producer emits `wording` Findings only when the direction deletes or replaces text at equal or smaller size, so this combination contradicts the producer's own rule. Reject and cite both field values. Not applicable when either field is absent.
...
| `Fix kind` / `Size delta` present or absent, any value | Not a parse-error — optional-recognized fields (v2 producer); captured when present, otherwise ignored |
...
| `marketplace.json` has no entry for the producer's plugin (`dev-workflow`, or file missing) at producer or consumer time | Treat the resolved version as `unknown`. Producer emits `**Producer version:** dev-workflow vunknown`; consumer's `producer_version == "unknown"` (or `current_version == "unknown"`) triggers Reject #7 stale-issue path on every Finding (still gated by the (i)+(ii) AND with doubt fall-through, so false rejects remain bounded) |
...
| Step 3.7 version-skew guard: `dev-workflow-bundle` is in the bump set AND the pre-bump `dev-workflow` version is strictly greater than `dev-workflow-bundle`'s (`bundle ≥ dev-workflow` passes; a bump set with no paired member runs no check) | Abort release bookkeeping with no marketplace / CHANGELOG edits; record `release-bookkeeping=failed (version skew: dev-workflow=<v1>, dev-workflow-bundle=<v2>)` |
...
| `dev-workflow` / `mobpro`-targeted Finding accepted (test-gated bundle members) | Edit target is under `skills/<target>/` (`mobpro` has no `references/`); (a.5) gate is `node --test tests/dev-workflow/skill-size.test.mjs` after the Edit, never `budget-deferred`; commits normally in § 3.4; (f.5) bundle copy sync runs (Bundle member `yes`); § 3.7 bumps paired with `dev-workflow-bundle` (paired CHANGELOG heading) |
