...
- **Step 8 (Code Review)** is capped at 1 iteration (`N_code = min(1, N_code)`) on any non-Trivial task, same override as Step 3. On a Trivial task, Step 8 stays skipped (`N_code=0`) — fast mode does not force it back on, and neither does it re-enable a code phase you configured to `0`.
...
| `review_iterations` | int \| map | `3` | Max iterations for Plan / Code Review. Scalar applies to both phases; map `{plan, code}` sets them independently; `0` turns that phase off |
...
- **Scalar** non-negative integer — applies the same cap to both phases (e.g. `review_iterations: 2`).
- **Map** `{plan: N, code: M}` — sets the Plan Review cap (Step 3) and the Code Review cap (Step 8) independently (e.g. `{plan: 1, code: 3}`). Each key must be a non-negative integer; an absent or invalid key (missing / negative / non-integer / wrong-type) falls back to default `3` for that phase only. Adopting the map form is an explicit opt-in; the scalar form and an absent `review_iterations` are fully backward-compatible.

**`0` turns a phase off.** Set `review_iterations: {plan: 0, code: 3}` to run the workflow without Plan Review, `{plan: 3, code: 0}` to run it without Code Review, or a scalar `0` for neither — name both map keys, since an absent key warns before falling back to `3`. The phase is then skipped exactly the way the Trivial tier skips it: its task rows are registered `completed` and the step passes straight through. Because it is your own declaration rather than something the workflow decided for you, it raises no Completion-summary skip reminder (unlike the difficulty-skip and `--fast` skips, which are always named there). Negative and non-integer values remain invalid and fall back to `3` with a warning.

Can be overridden for a single invocation with `-i N` / `--iterations N`, which **overrides both phases** with the same value regardless of the config form. `-i` takes a **positive** integer only — `-i 0` falls through to the configured `review_iterations` instead of turning a phase off for the run, since turning a phase off is a project-level decision.

Automatic adjustment by task difficulty (evaluated in Step 2) — the same cap is applied **independently to each phase's count** (they can differ only under the map form). Every tier only ever **lowers** a count, so a phase you configured to `0` stays `0` at every tier:
...
| Simple | Obvious bug fix, or a small feature addition that fully follows an existing pattern with no new design decisions | `N_plan = min(1, N_plan)`, `N_code = min(1, N_code)` | Step 6 Tidy + Step 6.5 Polish Prose + Step 7.5 Rules Compliance |
...
| 3 | Plan Review | Internal review by reviewer (up to N_plan iterations; skipped entirely when N_plan=0 — a Trivial task, any non-Trivial task under `--fast`, or a configured plan-phase `review_iterations` of `0`) |
...
| 7.5 | Rules Compliance Review | Verify `.claude/rules/` compliance via `rules-review` skill; skipped for Trivial / Simple tasks per the difficulty-skip matrix; under `--fast` it still runs but its own fix → re-verify loop is capped to a single pass. When the plan defers paired-change / bookkeeping edits (version bump, CHANGELOG) to the Step 10 commit gate, the dispatch says so, so the reviewer does not flag them as missing mid-run — a deterministic false positive that would otherwise need a hand-written exception every run. Step 8's deferred-verification gate injects the same scope note |
| 8 | Code Review | Code review by reviewer (up to N_code iterations; skipped entirely when N_code=0 — a Trivial task or a configured code-phase `review_iterations` of `0`; capped at 1 iteration under `--fast`) |
...
| `review_iterations` scalar is not a non-negative integer (or is neither scalar nor map) | Warns, uses default `3` for both phases (`0` is valid — it turns both phases off) |
| `review_iterations` map has an absent / negative / non-integer / wrong-type `plan` or `code` key | Warns, uses default `3` for that phase only (the valid key is kept; a key set to `0` is valid — it turns that phase off) |
