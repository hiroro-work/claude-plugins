...
**Fast-mode-skip reminder** (paired with the difficulty-skip reminder above, per the warning-string differentiation rule — a separate ledger keeps a fast-mode-caused skip from being misread as a difficulty-driven one): when `fast_mode_skipped_steps` (initialized at Step 1 sub-step 6's cross-step variable init table, populated by the run's `--fast` skip/cap sites — Step 1.5's plan-phase forcing / Step 6.5-only skip, plus the shared 1-pass cap, fired from whichever of Step 8's deferred aggregate pass or Step 7.5's `code_review_enabled: false` branch ran the cycle) is non-empty, surface a line in the resolved `language` naming the steps `--fast` skipped:
