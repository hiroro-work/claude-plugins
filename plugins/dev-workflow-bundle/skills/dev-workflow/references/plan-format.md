...
- [ ] **Permission and capability assumption verification**: if the plan's Approach or Design relies on a new external command, CLI flag, or permission grant executing without a confirmation dialog or access barrier — e.g., a prefix-form `allowed-tools` pattern assumed to auto-approve a command that carries write/delete flags, a new SDK/API assumed to behave in a specific way by default, or a deploy tool assumed to require no interactive credential step — verify that assumption from a primary source (official documentation, actual permission-matcher behavior, or hands-on test in the target environment) before building the Approach around it. If the assumption cannot be confirmed at planning time, add an explicit verification step to the plan or record it in Risks / Unknowns as unconfirmed.
...
**Path scope.** This two-tier presentation and its `ExitPlanMode` / approval-modal mechanism apply to the **`plan_mode_active == true`** path (`visual_plan_review: false` — `SKILL.md` Step 4 sub-step 2 path (a)). On the **`plan_mode_active == false`** path (`visual_plan_review: true` — path (b)) there is no Plan Mode plan file and no `ExitPlanMode`: the canonical plan document is `.claude/plans/<slug>.md`, and approval comes from the visual gate (browser submit) or, on fallback, the no-Plan-Mode chat reply. The chat output sequence below (the condensed view, steps 1–6) still applies to path (b)'s chat-approval fallback; only step 7's `ExitPlanMode` and the **Plan file** / **Mechanism dependency** items are path-(a)-specific.
