...
- feat: new plugin `dev-workflow-lite`
  ...
  - Self-Retrospective returns as an eighteenth phase with growth control on the producer side: at most three Findings per run, each naming a `behavior` or same-size `wording` fix with its character delta and the prose that pays for it, deduplicated against the target's current text; signals are judged from the run in context rather than a session-log scan. Reminder-only Findings are never emitted.
  ...
  - `SKILL.md` plus nine reference files total about 72k characters against `dev-workflow`'s 657k and `mobpro`'s 134k.
