...
- **language**: Optional. Output language code (e.g. `ja`, `en`) for the `Description` / `Suggested fix direction` paragraphs of Step 9.5 findings only. Everything else (schema tokens, terminal summary, destination header) stays English. Resolution: merged skill config → Claude Code settings (`~/.claude/settings.json` → `language` field) → default `ja`. `null` / empty string / non-string values fall through to the next resolution step. See `references/self-retrospective.md` §2.1 Language handling / §5 Contract note for the full scope contract. No observable effect unless `self_retrospective.feedback` is also configured AND the task is not assessed as Simple difficulty
