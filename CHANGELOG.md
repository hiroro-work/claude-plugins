...
- feat(dev-workflow, mobpro)!: remove the `interactive_commits` config key — the commit phase always runs
  ...
  - Skipping the deprecation notice and the calendar-anchored waiting period is a **deliberate deviation** from this repo's config-flag lifecycle. The user's removal decision came at the plan-approval gate rather than before planning, so the lifecycle's exception path is not met literally; the deviation is taken anyway on the grounds above. The CHANGELOG is the only notice: no tombstone warns a project whose config still carries the key.
  - 6,154 characters off the two skills' hot path, twice that counting the bundle copies.
