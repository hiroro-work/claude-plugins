...
- **§ Procedure step 3's per-round launch** — launching `crit --range <m6_review_base>..<unit_obj>` as **background Bash**, never the `Agent` tool (`SKILL.md` § Direct Agent dispatch sites). Substitute both endpoints that step otherwise takes from `interactive-commits.md`'s per-commit candidate: `<pre_round_head>` = `m6_review_base`, and `<round_commit>` = `<unit_obj>` from [`diff-review.md`](diff-review.md) § Per-unit review range.
...
- **§ Procedure step 6's "No machine iteration cap" note**.

## Decision mapping

M6 substitutes this for that file's **§ Procedure step 5's per-commit decision mapping**. The substitution also drops that step's per-round verification (that file's § Round verification and fix accumulation, which only that step enters) — M6 is not the commit phase, so it accumulates nothing for M11.5 either. Evaluate in order, first match wins:
