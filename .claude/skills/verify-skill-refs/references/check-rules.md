...
Canonical home for the detection rules this lint applies. `scripts/lint.mjs` implements everything mechanically decidable and a dispatched subagent judges the rest — § Executor pipeline draws the split.
...
1. Build a per-file **heading index** and a per-file **bold-span index** once (all `#`–`####` lines; every `**...**` span), each entry carrying its normalized forms. Both indexes take only lines **outside** fenced blocks, by the same mechanical fence filter as § Class (a)'s Extraction exclusions. A `§` key resolves against **either** index.
...
3. **Resolution scope model**:
   ...
   - **Unqualified form** (`§ <Heading>` alone) → resolve in the same file first, then `SKILL.md`, then any other file **of the same root**. A violation means the key resolves nowhere at all — `SKILL.md` legitimately cites headings that live in a references file (e.g. `§ User-gate summary preamble`).
...
A candidate that fails resolution is a **violation** (`class: "a"`) only when it is **unambiguous**: extracted from running prose, with a clean normalized key, and unresolvable everywhere. Demote to a warning (`class: "a-demoted"`) on any of these, each of which is decided mechanically. **Evaluate them in the order listed and report the first match**:

- **Sibling-root-only resolution** — the key resolves in a sibling root the reference does not name.
...
- **Exemplar context** — the candidate sits in a passage that documents the convention by quoting it. This cause is declared as the allowlist in `scripts/lint.mjs`, whose entries name a file plus a verbatim fragment. An entry an earlier cause outranks still counts as matched, so it is not reported stale.
...
- **Closed list**: only manifest-registered entries are checked. `scripts/lint.mjs` parses the tables below to count `checked.manifest_pairs` and to hand the rows to the judgment stage.
...
| localization-enumeration | `references/configuration.md`, the `language` bullet's enumeration of localized outputs (`SKILL.md` § Configuration's `language` bullet is now a one-line index into it) | `references/localization.md` § Localization granularity, opening "Applies to" sentence (`references/plan-format.md`'s same-named heading is now a delegating pointer to it) | The enumeration lives on the `configuration.md` side alone; `localization.md` carries an abbreviated gist plus a pointer naming that bullet as the exhaustive list. Divergence = the pointer missing, the pointer not naming its owning skill (`mobpro` reads this file at runtime and has a `references/configuration.md` of its own, so an unqualified path mis-resolves there), or the gist naming a category the `language` bullet does not |
| no-stall-gate-enum | `SKILL.md` § No-Stall Principle, user-gate enumeration, **together with** the tail-scoped continuation in `references/finish-phase.md` § Gates that its aggregate bullet points at (`dev-workflow` only; the two are one closed list) | Each bullet's named definition site ("defined in ..." pointer), plus every `USER APPROVAL GATE` marker in `SKILL.md` **and in `references/finish-phase.md`** | Every enumerated gate's definition site exists; every `USER APPROVAL GATE` marker has a corresponding enumeration bullet in whichever of the two halves covers it |
...
**Scope: changed lines only.** Unlike the other classes, this one is checked only over the lines a `--base-commit` diff reports as changed or added (plus untracked files in full), and is reported **not applicable** when no `--base-commit` was given — `checked.step_candidates` is then `0`.
...
3. Residue — a bare number whose sentence binds no descriptor — → warning `class: "c"` with the file and the sentence fragment. Regex-classifiable allowed forms are dropped by § Executor pipeline's mechanical stage; only the residue reaches judgment. This class is warning-only.
...
The machine counterpart of each root's `§ Phase naming in user-facing output` section: a literal string the tree tells the orchestrator to put in front of the user must not carry the root's identifier without naming what that phase does. **Status-affecting.**
...
Match names **case-insensitively** and on whitespace-normalized text.
...
- A backticked literal whose surrounding prose marks it as emitted: the verb set `render` / `emit` / `warn` / `present` / `report` / `surface` / `note` / `append`, with the verb in the run of prose **immediately before** the span rather than anywhere on the line. The backticked span is the candidate, and only a multi-word one.
...
**4. Demotion rule.** Report `class: "e-demoted"` (warning) instead when the candidate is itself an exemplar of the naming rule, which is the shared allowlist named in § Class (a)'s "Demotion rule (violation vs warning)" paragraph. When in doubt, demote.
...
Enumeration, extraction, indexing, class (a) resolution, class (e) judgment, class (c) pre-filtering, and every `checked` count are performed by `scripts/lint.mjs` before the judgment stage is dispatched. The rules in the class sections above are the specification that script implements. What it settles is not the judgment stage's to revisit.

**Narrowed extraction is reported, never silent.** An unclosed fence masks to end of file, and a line with an odd number of backticks has untrustworthy code-span pairing; each emits `class: "scope-narrowed"` with the file and line, so a swallowed region is distinguishable from a clean one. Reported for every tree the run loads, including one loaded only so that references naming it resolve. Warning-only, and emitted by the mechanical stage alone.
