...
>   - upper-level design alternatives — propose at least one alternative at the structural layer (e.g. the responsibility split), not only at the implementation level
...
>   - sibling-symmetry grep audit — when the plan adds a component sharing a label / identifier / surface text / domain concept with existing ones, `grep` for the siblings and tabulate firing conditions and side effects across new vs. existing; report any same-text-different-side-effect asymmetry as a finding
...
> - Write for the person consulting in plain words, one claim per sentence, with no figure of speech translated word for word
