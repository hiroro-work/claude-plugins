...
- **`run_mode == "fast"` on a non-Trivial tier** — use `Step 3 (Plan Review) was skipped because fast mode skips Plan Review for this run — this approval is the sole review.` instead.

**Rules-only conditional**: when `plan_review_enabled` is `true` and `plan_review_scope` is `rules-only` — the `normal` run mode's scope (§ Configuration's **Run mode** bullet) — Step 3 ran, but over `.claude/rules/` compliance alone — so the "The plan has been reviewed in Step 3 ..." sentence overstates what was reviewed. Two sentences, on whether the pass reached a reviewer at all:
