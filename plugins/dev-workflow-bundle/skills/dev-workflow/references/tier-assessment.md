...
- **`--fast` forced `plan_review_enabled = false` on a non-Trivial tier**: mark the top-level `Step 3: Plan Review` row `completed` and leave `Step 8: Code Review` / `Step 8-1` untouched. **Only when `plan_review_enabled` was `true` before the forcing**, append `Step 3 Plan Review skipped (fast mode)` to `fast_mode_skipped_steps`.
