...
- **Every incremental run now audits the rules it just wrote.** The extraction criteria ask whether a candidate reads as a rule to someone with no memory of what produced it, and the dispatch that writes the rules is holding the conversation, the PR comments, or the scan it derived them from — so the test never actually ran, and entries recording what one session happened to do kept landing. A new pass, `references/audit-pass.md`, re-reads the entries a run wrote from a separate analysis subagent that is given the written text and the criteria and barred from reading the run's input, then applies its verdicts. It runs in Conversation Extraction, Conversation Candidate Apply, Update, and PR Review; Full Extraction, Restructure, and Compaction do not run it, and `--realign` remains the way to put rules written by an earlier run through the same criteria.
- **The pass reuses Realign's judgement vocabulary rather than adding one.** Verdicts are Realign's closed `keep` / `drop` / `split` / `reshape` under its definitions, the `mechanical_edits` and response schemas follow its shapes, adding `file` to each `rules` entry, and dropped entries take the `.examples.md` follow-through Realign already specifies.
- **Nothing is dropped silently, and nothing waits on a person.** The run's report gains an `### Audit (N judged — …)` section listing every non-`keep` entry with a one-sentence reason, plus a not-applied line for an edit that did not land. There is no setting and no approval gate — `extract-rules` is called non-interactively from `dev-workflow` and `dev-workflow-triage`, and a gate would stall both. A parse failure or a schema violation reports itself and leaves the extraction's writes untouched rather than failing the run.
- **The write step records what it wrote, and that record is what gets audited.** `references/conversation-mode.md` § Step C5 item 4 — which PR Review and Conversation Candidate Apply both defer to — and `references/update-mode.md` Step U5 now keep one entry per bullet appended or promoted, holding the file's absolute path, the bullet verbatim, and the report section that will list it; item 8 returns it alongside the summary. `.examples.md` writes are not recorded, which is what keeps them out of the audit. The run's report renders the same writes and is not a second source.
- Sizes: `SKILL.md` 30,239 → 31,948 chars, against the 40,000 compaction threshold; the new `references/audit-pass.md` is 10,678 and is read only on the branch that runs the pass.
...
- **Three default-off procedures left the startup read path.** The crit commit gate moved to `references/crit-gate.md`, the plan artifact and its team-review round trip to `references/plan-artifact.md`, and the decomposition state file, resume, and subtask-finish procedures to `references/decomposition-state.md`. Each is read only on the branch that needs it — `commit_review_gate: crit`, `plan_artifact: share` or `review`, and a decomposed run — so a default run no longer reads them. Section bodies moved; every pointer to them names the new file.
- **Phase 2 reads the decomposition references per branch.** The unconditional read is gone: Resume sub-mode reads `references/decomposition-state.md`, the full lane reads `references/decomposition.md`, and an express-lane run reads neither. Phase 19 re-reads the state-file reference before finishing a subtask, which it previously relied on Phase 2 having loaded.
- **The skip table's Update Rules row said `skip` on Trivial and Simple, but its user gate runs on every tier.** The row now reads `gate only`; only the rule extraction is skipped, which is what the README already said.
- **Restated prose is stated once.** Among the cuts to the always-read files: the lane definitions and re-check rationale in `tiers.md`, the flag semantics in § Mode detection, the per-phase settings preconditions under the skip table, the approval-token examples and language rule in `commits.md`, and the rationale prefaces of `snapshots.md`, `review-launch.md`, and `timing.md`. Each survives at the one place that governs it. `plan-format.md` also loses its description of how the plan viewer renders Overview; the one contract inside it — that the viewer shows the Scope count as written — moves into the Scope field's own spec.
- **Self-Retrospective drops three references that only resolved inside the marketplace repository.** Removed: the budget gate naming the size tests (28,000 / 80,000 / 12,000, already stale against them), the working-directory clause that scoped the existing-rule `Grep` to a run inside this repository, and the `jq` read of `marketplace.json` for the producer version. A positive size delta still names the prose that pays for it, and the `Grep` now skips when the target's files cannot be located. The version line asks for the running plugin's version instead of naming a file to read it from; issues filed from outside this repository already carried a real version under the old wording. The issue body's shape is unchanged, so downstream triage parses it as before. `Bash(jq *)` leaves `allowed-tools` with its last caller.
- **The README drops its comparisons with the predecessor version.** The "What is not here" section, the `v1 step` column of the Phases table, and the three sentences that contrasted this skill with a separately-named one are gone; `boundary_check_commands` and `implementation_executor` go with them, as neither is a key this skill reads. The stale budget figures go too.
- **Mob mode's unit review now reads `crit-gate.md` before opening the crit browser.** The launch procedure — the once-per-run availability probe, and the rule that the review is over only once an `approved:` line appears — was unreachable from `references/mob-mode.md`, which reaches the crit surface at Implement, well before `commits.md` is read. Without it an empty completion read as a broken run and fell silently to the chat presentation.
- **Startup read shrinks.** A default run reads `SKILL.md` plus seven references, 56,250 chars, against 68,681 for the eight it read before; a full-lane run adds `decomposition.md` for 58,659. `SKILL.md` itself 29,195 → 28,985. The three extracted files carry a ratchet of their own so relocated text stays bounded, and `mob-mode.md`'s rises to 13,200 for the pointer named above.
