...
- feat: new plugin `dev-workflow-lite`
  ...
  - Rules Compliance Review and Code Review are launched in the background when Check / Test starts and collected afterwards; a result is used only when no file changed since launch, otherwise the review re-runs inline (`references/review-launch.md`). Mob mode launches them too, after its pre-review prediction; mobpro's non-adoption of the overlap had no recorded reason and the stale case costs one discarded dispatch, not correctness.
