...
- **Triage scope (6 targets)**: the 5 bundle skills `dev-workflow`, `ask-peer`, `extract-rules`, `rules-review`, `tidy`, plus `dev-workflow-triage` (self — project-local). Release bookkeeping (version bump / CHANGELOG, `§ Step 3.7 — Release bookkeeping`) covers the 5 bundle skills only; `dev-workflow-triage` is project-local (not registered in `.claude-plugin/marketplace.json`) and is excluded from bookkeeping by design
...
- Any Finding's `Target skill` is outside the triage scope (the 5 bundle skills `dev-workflow`, `ask-peer`, `extract-rules`, `rules-review`, `tidy`, plus `dev-workflow-triage` — see `§ Fixed configuration`)
