...
allowed-tools: Read, Edit, Write, TaskCreate, TaskUpdate, TodoWrite, Skill(verify-diff), Skill(skill-review), Skill(publicity-review), Skill(rules-review), Skill(verify-bundle-sync), mcp__github__list_issues, mcp__github__issue_read, mcp__github__add_issue_comment, mcp__github__issue_write, Bash(git diff *), Bash(git add *), Bash(git commit *), Bash(git reset), Bash(git checkout HEAD -- *), Bash(git fetch *), Bash(git push *), Bash(git rev-parse *), Bash(git config --get *), Bash(git for-each-ref *), Bash(git symbolic-ref *), Bash(git switch *), Bash(git branch *), Bash(date *), Bash(jq *), Bash(mkdir -p *), Bash(cp -R *), Bash(wc -m *), Bash(find *)
---
...
- **Triage scope (7 targets)**: the 6 bundle skills `dev-workflow`, `ask-peer`, `extract-rules`, `rules-review`, `tidy`, `mobpro`, plus `dev-workflow-triage` (self — project-local). Release bookkeeping (version bump / CHANGELOG, `§ Step 3.7 — Release bookkeeping`) covers the 6 bundle skills only; `dev-workflow-triage` is project-local (not registered in `.claude-plugin/marketplace.json`) and is excluded from bookkeeping by design
- **Edit target paths**: for the 6 bundle skills, always `skills/<target>/SKILL.md` or `skills/<target>/references/*.md` (canonical source of truth — the flat direct-skill layout: SKILL.md and `references/` live directly under the plugin source dir `skills/<target>/`) — never edit `.claude/skills/<target>/...` symlinks. For `dev-workflow-triage` (self), the canonical lives at `.claude/skills/dev-workflow-triage/SKILL.md` / `.claude/skills/dev-workflow-triage/references/*.md` (a project-local real directory, not a symlink)
...
- **GitHub access path**: every `SonicGarden/dev-workflow-issues` operation in this skill uses the four `mcp__github__*` tools listed in frontmatter (`mcp__github__list_issues`, `mcp__github__issue_read`, `mcp__github__add_issue_comment`, `mcp__github__issue_write`) — never `gh` CLI (a prior Claude Code on the Web spec change broke API-based GitHub access via `gh api`, which this skill used exclusively before). A prior version of this skill avoided GitHub MCP because Claude Code on the Web was believed to scope it to the active working repository, denying cross-repository calls; the operator confirmed that constraint is resolved under the same spec change, and `mcp__github__*` now reaches `SonicGarden/dev-workflow-issues` from a session working in a different repository (this is operator-confirmed, distinct from the parameter-schema uncertainty noted below, which remains unverified). No separate connectivity pre-check runs here — GitHub MCP reachability is validated implicitly by Step 2's `mcp__github__list_issues` call, which is already a fatal-abort exit (see `§ No-Stall Principle`). **Field/value names shown at each `mcp__github__*` call site in this skill are illustrative, not a decided-on schema** — this session had no live GitHub MCP connection to verify parameter names against, so use each tool's actual parameter schema at call time. The same uncertainty applies to failure detection at each call site's "the call throwing, or its response indicating failure" branch: treat a thrown exception, a response carrying an explicit error/message field, or a response missing the fields the operation's success case would populate, as failure — this skill cannot name the exact signal in advance.
...
- **Char-budget cache** (feeds § 3.4 sub-step (a.5)): for each of the 7 triage-scope targets, take **two** measurements. `<target-root>` is `skills/<target>` for the 6 bundle skills and `.claude/skills/dev-workflow-triage` for self.
  - **The budgeted base** — `chars_at_preflight[target]`: list the target's `SKILL.md` plus every `references/*.md`, excluding `README.md` (user documentation, not part of the maintained instruction surface), then total their chars. **Two commands, deliberately:** first `find <target-root> -type f -name '*.md' ! -name 'README.md'` to get the path list, then a single `wc -m <path> <path> …` over exactly those paths, reading its trailing `total` line (with one path `wc` prints no total line — read that line's count instead). **Keep `find` free of `-exec` / `-delete` / any other write-or-exec flag**: a `Bash(find *)` prefix grant does **not** auto-approve those, so an `-exec` form stops at a permission dialog on every run — and this call sits in Pre-flight, before any issue is processed, in a routine that is explicitly non-interactive. Piping `find` into `wc` is likewise avoided (the pipe-binary grant rule in `.claude/rules/project.rules.local.md`); the agent reads the path list and issues the `wc -m` call itself. **The whole tree, not `SKILL.md` alone, is the budgeted unit**: `§ Fixed configuration` lets a Finding's `Edit` land in a `references/*.md`, and a `SKILL.md`-only base leaves that growth entirely ungated. `find` does the traversal rather than a bare `skills/<target>/references/*.md` glob because a target with no `references/` directory would abort the command under zsh's `nomatch`.
  - **The headroom anchor** — `skill_md_chars[target]`: `wc -m <target-root>/SKILL.md`, that one file. Used **only** to size the per-run growth allowance: `headroom[target] = max(round(0.03 × skill_md_chars[target]), 2500)` (round half up, e.g. `jq -n --argjson c "$chars" '(0.03 * $c) | (. + 0.5 | floor)'`). **Keeping the anchor on `SKILL.md` is load-bearing.** The 3% constant was calibrated when `SKILL.md` was also the base; re-deriving it from the tree total would scale the allowance with the widening — for `dev-workflow` that is ~3.8k → ~20k per run, which both defeats the gate and starves `dev-workflow-monthly-consolidation`'s "`budget-deferred` count ≥ 3" trigger. Widening the base closes the loophole; widening the allowance would reopen it.

  Then `cap[target] = chars_at_preflight[target] + headroom[target]`. `cap[target]` is fixed for the whole run; § 3.4 sub-step (a.5) compares against a **fresh** re-measurement of the tree each time, not an incrementally-tracked running total (see (a.5) for why)
...
`Step 1: Pre-flight` is registered already-`completed` so the Step 4 audit trail can show "pre-flight passed" (the item-name column above is the Task tools' `subject` field; under the `TodoWrite` fallback it is `content`). After this registration burst, proceed directly to Step 2 in the same tool-call burst — the next tool call must be the `mcp__github__list_issues` invocation, not a summary turn. See `§ No-Stall Principle` § Phase / per-issue status transitions.

### Step 2 — List open issues

- Call `mcp__github__list_issues` for `owner=SonicGarden`, `repo=dev-workflow-issues`, an open-state filter, and a cap of 50 items (names/values illustrative — see `§ Step 1 — Pre-flight`'s "GitHub access path" note). The 50-issue cap is intentional — running the full per-Finding sub-flow (verify-diff + skill-review + publicity-review dispatches × outer loop) on hundreds of issues per routine invocation is impractical; subsequent routine runs progressively drain the rest of the queue. Build a `{number, title, body, labels}` record per issue from the response, dropping pull requests if the response includes them (only true issues are triaged)
- The call throwing, or its response indicating failure ⇒ fatal abort with summary "issue list failed" (covers auth revoked / network failure mid-run — GitHub MCP reachability is otherwise unverified for the duration of the run)
- **Budget-deferred pre-filter**: from the fetched array, split off every issue whose `labels` contains `budget-deferred` — these were already deferred by a prior run's char-budget gate (§ 3.4 sub-step (a.5)) and are awaiting headroom from a future consolidation pass. Do not create per-issue rows for them or re-run the per-Finding flow; record their count as `budget_deferred_skipped_count` (rendered at Step 4 as `budget-deferred (skipped, awaiting headroom): <count>`). The remaining issues (label absent) form the processable set that the counts below (`N`, the 0/1–49/50 branches) are computed over. This is a **collection-stage pre-filter** (a Step 2 exclusion decided before any issue enters the per-issue loop), not a per-Finding disposition — it belongs at this layer because the exclusion reason (no headroom exists yet) applies to the whole issue uniformly and is already known before Step 3's per-Finding judgment would otherwise re-derive it every run
- **Body backfill**: for each issue in the processable set (after the budget-deferred pre-filter above — never for excluded issues) whose `body` came back absent from `mcp__github__list_issues`, call `mcp__github__issue_read` to fetch the full body. These per-issue calls have no ordering dependency on each other (Step 3's per-issue loop, which does have same-file-edit ordering constraints, has not started yet), so they may be issued in the same tool-call burst rather than one at a time
- Empty (`0` issues, after the budget-deferred pre-filter) ⇒ in **a single tool-call burst** (one `TaskUpdate` per flip; or a single `TodoWrite` call under the fallback), flip `Step 2: List open issues` → `completed`, `Step 3: Process each issue serially` → `completed` (no work to do), `Step 3.7: Release bookkeeping` → `completed` (the "skipped (no commits)" branch is reached by definition with zero issues), `Step 3.8: Rules-compliance detection` → `completed` (no run diff to review — reached by definition with zero commits), and `Step 4: Emit summary` → `in_progress`; then proceed directly to summary emission in the next tool-call burst. The hazard `§ No-Stall Principle` guards is a turn boundary or user-facing prose inserted between the flips and the next action — not the number of tool calls; multiple `TaskUpdate` calls within the same burst (same response) do not open a pause-window. Summary text begins with "no open issues". The empty-issue path is the **normal path**, not a fatal-abort exit (those are limited to the two cases in `§ No-Stall Principle`): it does **not** terminate at the Step 4 summary. Per `§ Step 4 — Emit summary`'s "Return-point no-stall reminder", the same burst that emits the summary also flips `Step 5: Self-retrospective` → `in_progress` and the run proceeds into `§ Step 5 — Self-retrospective`, which emits `self-retrospective: skipped (no observations)` on this zero-observation run. The `Step 5: Self-retrospective` row is flipped by that Step 4 boundary reminder, not in this Step 2 enumeration
...
- Exactly `50` ⇒ same per-issue append + status flip as above, plus set `overflow=true` (surface in summary as "50-issue cap reached" — prefer the tool's own pagination/total-count metadata when the response carries it; otherwise fall back to a full page of `== 50` as the overflow signal)
...
- **Normal path** (§ 3.2 parse OK → § 3.3–§ 3.4 → § 3.5 → § 3.6): per-issue row flips to `completed` immediately after Step 3.6's Close decision settles (success or failure alike — `close-failed` does not block the row flip)
...
- Any Finding's `Target skill` is outside the triage scope (the 6 bundle skills `dev-workflow`, `ask-peer`, `extract-rules`, `rules-review`, `tidy`, `mobpro`, plus `dev-workflow-triage` — see `§ Fixed configuration`)
...
- `producer_version == current_version`: standard checklist only — no extra version-aware step (this skips only Reject #7's CHANGELOG-range + (i)/(ii) legs; the standard checklist still requires the target-file `Read` mandated at the top of `§ 3.3 Judge each Finding` — "read the target's SKILL.md first" — that Reject #1's "no longer reproducible" judgment depends on)
...
Self-retrospective issues (`Target skill: dev-workflow-triage`, filed by `§ Step 5 — Self-retrospective`) intentionally omit the Producer version line, so they always enter this path with `producer_version = "unknown"`. Leg (i) cannot hold for them (`dev-workflow-triage` is excluded from release bookkeeping and never appears in `CHANGELOG.md`), so judgment falls through to the standard checklist — the intended behavior; Reject #1's already-addressed check still works from the current-file read.
...
- `disposition`: default `accept`. Downgrade paths in (a.5)/(b)/(c)/(d)/(d3)/(f)/(g) overwrite it to `conflict`. Final values: `accept` / `reject` / `conflict` / `parse-error`.
...
- **(a) Re-read target file** — `skills/<target>/<file>` for bundle skills, `.claude/skills/dev-workflow-triage/<file>` for the self target (per `§ Fixed configuration`'s Edit target paths). For the first Finding this is HEAD state; for later same-file Findings it's the prior commit's result. Build `old_string` / `new_string` against this state.

  **Per-Finding input binding (mandatory)**: At (a) entry — before any (a.5) char-budget check, (b) Edit, (c) frontmatter check, or (d-loop) callee dispatch — bind `record.description` and `record.suggested_fix_direction` to the verbatim text of this Finding's `**Description:**` and `**Suggested fix direction:**` fields parsed by § Parse body. Single immutable read scoped to **this** `### Finding <n>` section. Every subsequent `Skill(verify-diff)` dispatch in the (d-loop) for this Finding MUST source its `Description` / `Suggested fix direction` arguments from `record.description` / `record.suggested_fix_direction` — never from re-reading the issue body, never from paraphrase, never from another Finding's record. Cross-Finding text contamination (e.g. using issue #M Finding K's `Description` while processing issue #N Finding L) is a Critical-severity routine bug; the binding-at-(a) discipline structurally prevents it by establishing exactly one canonical source per Finding. `publicity-review` does not take these inputs (per its `## Invocation contract`), so this binding applies only to verify-diff.
- **(a.5) Char-budget check** — before drafting (b)'s `Edit` call, re-measure the target's whole tree with the budgeted-base command § 1 Pre-flight's Char-budget cache defines, to get `current_chars[target]` — a **fresh** read of the tree's actual current state, not an incrementally-tracked running total. This is deliberate: a prior same-target Finding may have left uncommitted residue in the working tree (the (d) `conflict` branch does not revert (b)'s edit — see (d)'s `conflict` semantics), so only a fresh measurement is guaranteed to reflect what is actually on disk right now. Measuring the tree also means a Finding whose `Edit` lands in a `references/*.md` is budgeted the same as one landing in `SKILL.md`. Then estimate this Finding's naive char delta: `Σ (chars in new_string) − (chars in old_string)` across the fix's planned edits — count the planned `new_string` / `old_string` text directly (they are still in-memory `Edit` arguments, not yet on disk, so no `wc` invocation applies here; count in the same character unit `wc -m` uses). Compare `current_chars[target] + delta` against `cap[target]` (from § 1 Pre-flight's Char-budget cache).
  - **Within cap** — proceed to (b) with the drafted edit unchanged.
  - **Over cap** — where the fix admits a **consolidation-form** redraft (folding the new content into an existing nearby reminder / paragraph via cross-reference instead of adding freestanding prose), redraft and re-estimate:
    - **Within cap after redraft** — proceed to (b) with the redrafted edit.
    - **Still over cap** — downgrade to `conflict` and continue: skip (b)–(g) for this Finding, set `record.disposition = conflict` (`record.target` stays at its default `—`, since (b) — the only sub-step that sets it — never runs), record warning `skill-tree-growth budget exceeded (<projected> > cap <cap>) — deferred`, and mark the Finding for `budget-deferred` label application at § 3.5 Post triage comment.
  - A Finding whose nature admits **no** consolidation-form redraft (a net-new addition with no existing site to fold into) follows the same "still over cap" disposition directly.
...
- **(d-loop) Outer review loop** (max 3 iterations, early-exit on zero edits or callee abort):
  ...
  **Notes**:
  ...
  - **Per-Finding task rows** — `(d) verify-diff call`, `(d2) skill-review call`, `(d3) publicity-review call` flip to `in_progress` on iter 1's call and to `completed` on the last iter's call (whether early-exit, callee-abort break, or max-iter). They are not re-flipped per iteration. On the (d) `conflict` callee-abort path — where downstream `... call` rows (d2)/(d3) are never reached (because (d) `conflict` breaks the outer loop on iter 1 step 2 before (d2)/(d3) ever run) — mark the (d2) and (d3) rows `completed` with the reason appended to the task's `description` field (the `content` field under the `TodoWrite` fallback) as `— skipped: callee-abort (d) verify-diff conflict`. **On a Finding that downgrades to `conflict` before (d) is ever called** — the (a.5) still-over-cap budget-defer path, a (b) Edit failure, or a (c) frontmatter-integrity failure — mark **all three** `... call` rows `completed` in the same tool-call burst that records the downgrade, with the reason appended as `— skipped: pre-(d) conflict (<sub-step>)` (substituting `a.5` / `b` / `c` for `<sub-step>`); none of the three callees ever ran for this Finding. This wording is distinct from the `— skipped: disabled` reason used for the consecutive-error disable bypass and from the `— skipped: callee-abort (d) verify-diff conflict` reason above; the (d3) `unresolved`/`conflict` path does not need any of this treatment because (d), (d2), and (d3) have already run on the iter that broke.
...
- **(d) `Skill(verify-diff)` empirical check** — `verify-diff` derives 1–2 evaluation scenarios from the Finding in its main thread, then dispatches a fresh bias-free executor that actually runs those scenarios against the post-diff file; if gaps remain the executor returns `suggested_edits` as JSON and `verify-diff` applies them autonomously. The callee runs its own internal iteration loop (Pattern A, default max 1 — a single dispatch); the orchestrator-side outer loop (see (d-loop) above) re-dispatches this callee up to 3 times per Finding.
...
- **(d3) `Skill(publicity-review)` empirical check** — `publicity-review` dispatches a fresh subagent against the diff and applies its `suggested_edits` via Edit, returning a single fenced JSON verdict at end. The callee runs its own internal iteration loop (Pattern A, default max 1 — a single dispatch); the orchestrator-side outer loop (see (d-loop) above) re-dispatches this callee up to 3 times per Finding.
...
- **(f.5) Bundle copy sync** — workaround for upstream Claude Code symlink bug ([anthropics/claude-code#53948](https://github.com/anthropics/claude-code/issues/53948); to be removed together with the `verify-bundle-sync` skill once the bug is fixed). Runs only when `record.target` matches `skills/<bundle-name>/...` where `<bundle-name>` ∈ {`ask-peer`, `dev-workflow`, `extract-rules`, `rules-review`, `tidy`, `mobpro`}; skipped for non-bundle skills and on the (b)/(c)/(d)/(d3) callee-abort downgrade paths (since (f) is already skipped on those). Self-targeted Findings (`record.target` under `.claude/skills/dev-workflow-triage/`) never match this pattern, so (f.5) is skipped for them by construction.
  ...
  3. On `status: "drift"`: run `cp -R skills/<bundle-name>/. plugins/dev-workflow-bundle/skills/<bundle-name>/` to propagate the Finding's edits from canonical to bundle copy, then `git add plugins/dev-workflow-bundle/skills/<bundle-name>/` to stage the bundle copy paths so (g)'s commit captures both. Proceed to (g).
...
- **(g) Commit** — use a HEREDOC with sentinel `COMMIT_MSG_END` (not `EOF`, to avoid early termination if Finding text contains an `EOF` line):
  ...
  On zero exit: capture `git rev-parse HEAD` for the summary, set `record.commit = <hash>` and `record.disposition = accept`, and increment `triage_commit_count` by 1. No char-budget bookkeeping is needed here — (a.5) always re-measures the target tree fresh, so there is no running total to advance. On non-zero (typically a pre-commit hook rejection): run `git reset` + `git checkout HEAD -- <paths>` to return to a clean tree, downgrade to `conflict` (`record.disposition = conflict`, `record.commit = —`), record `commit-failed`, continue without incrementing `triage_commit_count`
...
**Tool path**: use the four `mcp__github__*` tools listed in frontmatter for all `SonicGarden/dev-workflow-issues` operations (see `§ Step 1 — Pre-flight`'s "GitHub access path" note).
...
- `Read` the staging file's content, then call `mcp__github__add_issue_comment` for issue `<N>` with that content as the comment body (names/values illustrative — see `§ Step 1 — Pre-flight`'s "GitHub access path" note)
- The call throwing, or its response indicating failure: record `comment-failed`, continue with other issues
- **Budget-deferred label**: if any Finding in this issue was deferred by § 3.4 sub-step (a.5)'s char-budget gate, apply the `budget-deferred` label: call `mcp__github__issue_read` to fetch issue `<N>`'s current labels, then call `mcp__github__issue_write` (update) with the existing labels plus `budget-deferred` (de-duplicated) — read-then-write, since a bare write of just `budget-deferred` risks silently replacing the issue's other labels if the tool's `labels` field is full-replace rather than additive. Non-fatal on failure — record `label-failed` and continue; this label is consumed by § 2 List open issues' pre-filter on a future run, not required for this run's own disposition
...
- When closing, call `mcp__github__issue_write` (update) for issue `<N>` with a closed state and a reason of `completed` (any accepts) or `not_planned` (all rejects) (names/values illustrative — see `§ Step 1 — Pre-flight`'s "GitHub access path" note)
- The call throwing, or its response indicating failure: record `close-failed`, continue
...
**(b) Modified-skill set**: from the accepted-and-committed list, build the unique set of `<target-skill>` values. Filter against the six-skill bundle whitelist (`dev-workflow`, `ask-peer`, `extract-rules`, `rules-review`, `tidy`, `mobpro`). This filter is also the **by-design exclusion point for `dev-workflow-triage` self-fix commits**: the self target is project-local (no marketplace plugin entry, no CHANGELOG entry), so it must not enter the bump set — accepted self-targeted Findings are committed in § 3.4 but deliberately dropped here.

**(c) Plugin mapping**.

> **Reminder — keep in sync with `§ Fixed configuration`**: This must list exactly the 6 bundle skills from `§ Fixed configuration`'s "Triage scope" line (the bundle subset — `dev-workflow-triage` is intentionally absent from this table: project-local, excluded from bookkeeping per (b) above). **Source of truth for bundle membership is `marketplace.json`'s `dev-workflow-bundle.skills` array.** When adding or removing a bundle skill, sweep **every** bundle-member enumeration in one commit — in this file: the frontmatter `description`, `§ Fixed configuration`'s "Triage scope" + "Edit target paths", the `§ Parse body` out-of-scope reject criterion, the Step 3.3 read-target line, § 1 Pre-flight's Char-budget cache line (its 7-target / 6-skill counts), `(f.5)` Bundle copy sync's `<bundle-name>` set, `(b)`'s bump whitelist, this `(c)` table, and `(e)`'s jq `IN(...)` example; in `references/triage-criteria.md`: the edit-target-dir check and the two out-of-scope reject criteria. `.claude/rules/project.rules.md` is **not** a sweep target: its bundle-skill cp -R sync rule and paired-bump rule name `marketplace.json`'s `dev-workflow-bundle.skills` array as the membership authority instead of enumerating members, so they track membership changes on their own. Locate the prose sites with `git grep -nE 'bundle skills|six-skill'`; the `(c)` table and `(e)` jq `IN(...)` filter sit just below in this same `§ Step 3.7` and list plugin names directly (no anchor phrase — update them in place).
...
```bash
jq -r '.plugins[] | select(.name | IN("dev-workflow","peer","extract-rules","rules-review","tidy","mobpro","dev-workflow-bundle")) | "\(.name)=\(.version)"' .claude-plugin/marketplace.json
```

(Filter the `IN(...)` list to just the plugin names in the bump set plus the always-included pair `dev-workflow` and `dev-workflow-bundle` — no need to read versions you won't bump or use for skew check.) Parse the `dev-workflow` and `dev-workflow-bundle` versions into integer 3-tuples (same `\d+\.\d+\.\d+` → integer-3-tuple parse as `§ 3.3 Judge each Finding`'s "Version-aware judgment" paragraph; **do not string-compare** — `1.69.0` vs `1.7.0` mis-orders lexicographically). **Abort only when `dev-workflow`'s 3-tuple is strictly greater than `dev-workflow-bundle`'s**: record `release-bookkeeping=failed (version skew: dev-workflow=<v1>, dev-workflow-bundle=<v2>)` and proceed to Step 4 (per-Finding commits stay in HEAD). `dev-workflow-bundle ≥ dev-workflow` is the steady state — the bundle bumps on every member change while `dev-workflow` bumps only when it itself changes, so the bundle leads or ties — and must **not** abort. The comparison stays scoped to `dev-workflow` vs `dev-workflow-bundle` only and is **not** generalized to all bump-set members: each member carries an independent version lineage (e.g. `peer` may have a higher major than the bundle), so a generalized `any member > bundle` check would false-abort permanently.
...
> **Return-point no-stall reminder**: Step 3.7 termination (regardless of outcome — a successful commit hash, `skipped (no commits)`, `failed (version skew | json invalid | changelog edit error | scope leak | commit error)`, any non-error result) is not a terminal point. Mark `Step 3.7: Release bookkeeping` row `completed` and `Step 3.8: Rules-compliance detection` row `in_progress` in the **next tool call** (one tool-call burst carrying both flips — two `TaskUpdate` calls, or a single `TodoWrite` call under the fallback), then proceed directly to Step 3.8. See `§ No-Stall Principle`.

### Step 3.8 — Rules-compliance detection (after release bookkeeping)

After Step 3.7, run a single `Skill(rules-review)` pass over **this run's diff** so any `.claude/rules/` violation the triage edits introduced — most importantly a bundle-skill change that landed without the paired version bump / CHANGELOG entry (which only appears in the diff once Step 3.7's bookkeeping commit has landed) — is detected on the production side and surfaced in the Step 4 summary. This runs **once per run**, not per issue. `rules-review` is **detect-only** (its `allowed-tools` has no `Edit`), so it never writes to the working tree. This is the LLM-based production detection layer; it complements `triage-review`'s `main..HEAD` cumulative backstop (the per-run base here reviews only this run's contribution, where the version-bump pairing lives).

Entry state: the `Step 3.8: Rules-compliance detection` row is already `in_progress` (flipped by Step 3.7's boundary reminder). When this step terminates (any branch — including the (a) gate skip), the boundary reminder at the bottom of this section handles the `completed` flip and the Step 4 `in_progress` flip in one tool-call burst. (The 0-open-issues path in Step 2 does not enter this section — Step 3.8 was already flipped to `completed` there as part of the five-row flip.)

**Ordering invariant — runs before auto-cleanup / push.** Step 3.8 must run **before** `§ Auto-cleanup of empty triage branch` and `§ Push triage branch to origin` (both inside Step 4): auto-cleanup switches back to `original_branch` and deletes the triage branch, after which the run's commits are no longer on HEAD to diff.

**`rules_review_result` State-variable contract** (mirrors the per-Finding record discipline — all fields initialized at entry so an early-skip path leaves an explicit default): at Step 3.8 entry, initialize `rules_review_result = "skipped (no commits)"`. The only write site is sub-step (c)'s parse; the only read site is the Step 4 `rules-review:` summary line. On the (a) skip path the default is left untouched, so the Step 4 render is well-defined without a dispatch.

(a) **Gate on commits**: if `triage_commit_count == 0` (no per-Finding or bookkeeping commit landed this run — the same condition that triggers `§ Auto-cleanup of empty triage branch`), **do not dispatch** `rules-review`: there is no run diff to review. Leave `rules_review_result = "skipped (no commits)"` and proceed to the boundary reminder at the bottom of this section. Otherwise continue to (b). (This gate is intentionally dev-workflow-triage-specific: `triage_commit_count` is already on hand for auto-cleanup, so skipping the dispatch on a 0-commit run avoids a needless subagent load. `triage-review` deliberately has **no** such gate — it dispatches unconditionally on its non-empty `main..HEAD` path and relies on `rules-review`'s own empty-diff → `no-issues` mapping for the trivial case; do not "align" the two by adding or removing a gate.)

(b) **Pre-invocation reminder**: the next tool call is `Skill(rules-review)` dispatch. Its return is a single fenced JSON verdict (`rules-review` § Return contract) — parse it as a structured return value per (c) below, and **proceed to the Step 4 transition in the same response**. Specifically forbidden between the verdict-parse and the next action: user-facing pause phrases per § No-Stall Principle, prose summaries of the rules-review verdict that end without proceeding, re-rendering the JSON block as a standalone deliverable. See `§ No-Stall Principle`.

(c) **Dispatch and parse**: invoke `Skill(rules-review)` with `--base-commit <triage_branch_base>` (the per-run base captured in Step 1 Pre-flight; `triage_branch_base..HEAD` is exactly this run's contribution). No `Model:` is passed (project-local routine, no model config). Parse the verdict with the first-match-wins evaluate-in-order discipline from `.claude/skills/verify-diff/SKILL.md` § (b) Parse & apply (restricted to single-pass dispatch — Converged / Divergence are N/A):

1. `Skill(rules-review)` could not be resolved / dispatched in this environment (skill-not-found, or no usable response at all) → `rules_review_result = "skipped (unavailable)"` (distinguishes "skill absent from this environment" from "contract broken", mirroring the `prompt-tuning skipped (agent unavailable)` precedent in `triage-review`)
2. The skill ran but the response carries no parseable fenced JSON verdict block → `rules_review_result = "error (verdict parse failure)"`
3. Schema violation (`status` outside `no-issues` / `violations` / `error`, or `violations_count` not a non-negative integer) → `rules_review_result = "error (verdict schema violation)"`
4. Otherwise — `rules_review_result =` `"no-issues"` / `"violations (<N>)"` (using `violations_count`) / `"error (<reason>)"` (a JSON `status: "error"` carries `rules-review`'s closed-enum `reason`: `diff collection failed` / `rule loading failed` / `verdict parse failure` / `coverage gap only`)

All outcomes are non-fatal (recorded, never an abort — this section adds no fatal-abort exit). **No consecutive-error disable counter** is kept: this is a once-per-run single dispatch, not an iteration loop, so it introduces no new scale-management gate (the `§ No-Stall Principle` scale-management gate list is unaffected).

After this step terminates (any branch), apply the following boundary reminder before proceeding to Step 4:

> **Return-point no-stall reminder**: Step 3.8 termination (regardless of outcome — `no-issues`, `violations (<N>)`, `error (<reason>)`, `skipped (no commits)`, `skipped (unavailable)`, any non-error result) is not a terminal point. Mark `Step 3.8: Rules-compliance detection` row `completed` and `Step 4: Emit summary` row `in_progress` in the **next tool call** (one tool-call burst carrying both flips — two `TaskUpdate` calls, or a single `TodoWrite` call under the fallback), then proceed directly to summary emission. See `§ No-Stall Principle`.
...
- `publicity_review` ∈ {`converged-iter-<k>`, `unresolved-<count>`, `conflict (<reason>)`, `skipped (<reason>)`} (the (d3) sub-step ran): render `<token> [iter <iterations_used>/1]`
...
issue #<N> Finding <n>: <accept|reject|conflict|parse-error>
  ...
  verify-diff:   <converged|unresolved|skipped|conflict> [iter <iterations_used>/1] | disabled [iter —] | —
  skill-review:  <no-actionable-findings|applied-edits|notes-left|error|disabled> | —
  publicity:     <converged-iter-<k>|unresolved-<count>|conflict (<reason>)|skipped (<reason>)> [iter <iterations_used>/1] | disabled [iter —] | —
  commit:        <hash> | —
```
...
- `budget-deferred (skipped, awaiting headroom): <count>` — issues excluded by § 2 List open issues' budget-deferred pre-filter (`budget_deferred_skipped_count`); omit the line when the count is `0`
...
- `skill-tree-growth: <target>: +<N> chars (cap <cap>)` — one line per target with ≥1 committed Finding this run. Source of truth: a fresh re-measurement of the target's whole tree (§ 1 Pre-flight's Char-budget cache budgeted-base command) against `chars_at_preflight[target]` (the Pre-flight cache), taken at Step 4. Omit the line for targets with no commits this run
- verify-diff state: `enabled` or `disabled-after-errors`; count of Findings with `verify-diff unresolved (<n> gaps)`; count of Findings with `verify-diff skipped (<reason>)` broken down by reason. Read the `unresolved` count as an **edit-activity rate, not an anomaly rate**: `verify-diff` reports `unresolved` whenever its final iteration applied edits with no successor iteration to re-verify them (`verify-diff` SKILL.md § Step 4 — Max iterations reached without convergence), which under its default `Max iterations` of `1` is every productive pass. `publicity-review`'s counterpart count keeps its anomaly reading — that callee drops the findings its own edits fixed
- verify-diff dispatches consumed: cumulative Agent-tool dispatches across the run (cost observability)
- skill-review state: `enabled` or `disabled-after-errors`; count of Findings with `skill-review notes left after applied-edits` (residual structural notes when terminal verdict is `applied-edits`); count of Findings with `skill-review notes left after max iters` (terminal verdict is `notes-left` — the callee's internal loop ended with zero applied edits and structural notes outstanding; at its default `Max iterations` of `1` that is one pass that applied nothing, not a callee that retried and gave up). Both are sub-counters of the overall "skill-review notes left" condition
...
- Failure counts: `comment-failed` / `close-failed` / `commit-failed` / `label-failed`, with (issue#, finding#) pairs
...
- `rules-review`: one of `no-issues`, `violations (<N>)`, `error (<reason>)`, `skipped (no commits)` (no run diff to review — `triage_commit_count == 0`), or `skipped (unavailable)` (the `rules-review` skill could not be resolved in this environment). Source of truth: `§ Step 3.8 — Rules-compliance detection`'s `rules_review_result` (initialized to `skipped (no commits)`, overwritten only by Step 3.8 (c)'s verdict parse) — render `rules-review: <rules_review_result>`
...
> **Return-point no-stall reminder**: Emitting the Step 4 summary (regardless of content — zero-activity, `push-failed`, any non-error result) is not a terminal point on the normal path. The same tool-call burst that prints the summary also flips the `Step 4: Emit summary` row `completed` (per the entry-state paragraph above) and the `Step 5: Self-retrospective` row `in_progress`, and the next tool call proceeds directly into `§ Step 5 — Self-retrospective`'s tally. See `§ No-Stall Principle`.

### Step 5 — Self-retrospective

Runs **once per run**, immediately after the Step 4 summary emission, on the normal path only. The two fatal-abort exits listed in `§ No-Stall Principle` stop before the per-issue loop — no callee dispatches ran, so there is nothing to observe and this step is skipped. Phase-row handling on those exits follows the existing abort behavior: a Step 1 abort stops before row registration, so the `Step 5: Self-retrospective` row never exists; a Step 2 abort leaves downstream rows un-flipped, same as the existing Step 3 / Step 3.7 rows.
...
Collect the run's observation signals from the conversation context. No state variables are maintained mid-run for this — the context itself is the evidence (see `§ No-Stall Principle`'s "Operator interventions are tallied at Step 5" paragraph for why mid-run recording is forbidden):

1. **Mid-run user messages**, classified into two buckets:
   - **Stall intervention**: a continuation nudge carrying no new directive content (e.g. 「続けて」, "continue", a restatement of the routine instruction). Record the callee return point / sub-step that immediately preceded the message as its `position` (granularity is sub-step level — outer-loop iterations of the same sub-step are not distinguished, so repeat interventions at the same sub-step aggregate into one Finding). These are the filing trigger.
   - **Directive**: a message carrying new instructions (a change of direction, an additional task). Not stall evidence — excluded from the filing trigger; only the count is mentioned in the Run context section.
...
- **Zero stall interventions** → do not file an issue. Emit the terminal line `self-retrospective: skipped (no observations)` (the normal, expected outcome) and finish the run.
- **One or more stall interventions** → assemble and file the issue below.
...
- **No `**Producer version:**` line** — intentional: this project-local skill has no marketplace version. The consumer treats the absence as `producer_version = "unknown"` and falls through to the standard checklist per `§ 3.3 Judge each Finding`'s self-retrospective paragraph (the intended degradation)
- One `### Finding <n>` per distinct stall position (multiple interventions at the same position are aggregated into one Finding with the count stated), each with the 4 labeled fields:
  ...
  - `**Category:**` — judged from content within the 5-value enum; stall observations are typically `ambiguity`, while mechanism defects are typically `other`
  - `**Description:**` — position, count, and an abstracted note of the intervention
  - `**Suggested fix direction:**` — MUST be written as a **concrete local edit**: name the target section heading and the wording to strengthen, at a granularity the next run can apply as a single `Edit` call. An abstract direction ("improve the no-stall wording") gets rejected under Reject #3 (too abstract) — concreteness is what closes the self-fix loop
- `## Run context` section (supplementary, outside the Findings — `§ 3.2 Parse body` reads only `### Finding` headings and labeled fields, so this section does not interfere with parsing): stop-hook fire count and positions, directive message count, `*_disabled` flag states, `triage_branch_name`, issue counts processed
...
- Title: `[triage-self-retrospective] dev-workflow-triage: <N> stall observation(s) (<triage_branch_name>)` — the branch name's timestamp doubles as the run identifier, so same-day re-runs never collide. Keep the prefix in sync with `§ Fixed configuration`'s "Self-retrospective issue title prefix" line
- `mkdir -p .triage`, then `Write` the body to `.triage/self-retrospective-<YYYYMMDD-HHMMSS>.md`. The file is gitignored and kept as a local in-session reference (the GitHub issue is canonical); do not delete it — same staging discipline as `§ 3.5 Post triage comment`'s staging-file step
- `Read` the staging file's content, then call `mcp__github__issue_write` (create) with the title and that content as the body (names/values illustrative — see `§ Step 1 — Pre-flight`'s "GitHub access path" note)
- If the call throws, or its response indicates failure, retry once after a 1–2 second sleep. On the second failure, record `self-retrospective-failed (<reason>)` where `<reason>` is the tool's returned error message truncated to ≤ 80 characters; if no error message is available, render `self-retrospective-failed (no error detail)`. Do **not** auto-recover (no alternative endpoint, no destination rewrite); the staging file stays in place for a manual retry post-run

#### Terminal line

Always emit exactly one of the three closed forms below (structured tokens — preserved verbatim per `§ Output language`), in the same tool-call burst as the `Step 5: Self-retrospective` row's `completed` flip. This is the true end of the run:

- `self-retrospective: filed #<issue-N>`
- `self-retrospective: skipped (no observations)`
- `self-retrospective: failed (<reason>)` — the rendered form of the recorded `self-retrospective-failed (<reason>)` token (same `<reason>` string; the recorded token is what `§ No-Stall Principle`'s non-fatal enumeration and the edge-case dispatch table name)
