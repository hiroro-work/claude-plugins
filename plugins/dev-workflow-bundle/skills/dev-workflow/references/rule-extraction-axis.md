...
6. **Abstraction normalization** → normalize phrasing so the main sentence generalizes and the rule can be re-matched in a later session (enabling staging → canonical promote). Incident-specific detail is **dropped**, not relocated into a parenthetical suffix; retain one parenthetical only when the main sentence alone does not say where the rule applies. A rule whose main sentence is incident-specific will never re-match.
