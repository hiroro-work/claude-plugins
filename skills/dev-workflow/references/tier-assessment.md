...
Mark every row the tier skips `completed` here. **On the progress-file surface** (`references/step1-load-settings.md` § Sub-step 7 — registration mechanics' progress-file paragraphs) that mark is written as the `skipped (<cause>)` line rather than a `completed` one, so the skip stays distinguishable from a phase that ran; the two are equally settled, and every reader treats them so. Entry-side handling is not this section's — each such row is recognized at its own step by `SKILL.md` § Step 1 registration mechanics' **Pre-completed row guard**.
