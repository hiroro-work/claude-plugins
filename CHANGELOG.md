...
- feat: new plugin `dev-workflow-lite`
  ...
  - Dropped, as features outside the workflow itself: mid-run difficulty escalation after Implement, `implementation_executor`, `boundary_check_commands`, Workability Retrospective. Post-Commit Verification is folded into Interactive Commits as one rule.
  ...
  - `SKILL.md` plus ten reference files total about 77k characters against `dev-workflow`'s 657k and `mobpro`'s 134k.
