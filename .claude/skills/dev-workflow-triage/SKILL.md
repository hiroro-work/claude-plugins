...
- **Char-budget cache** (feeds § 3.4 sub-step (a.5)): for each of the 7 triage-scope targets (the 6 bundle skills' `skills/<target>/SKILL.md`, plus `.claude/skills/dev-workflow-triage/SKILL.md` for self), run `wc -m` and store the result as `chars_at_preflight[target]`. From it derive `headroom[target] = max(round(0.03 × chars_at_preflight[target]), 2500)` (round half up, e.g. `jq -n --argjson c "$chars" '(0.03 * $c) | (. + 0.5 | floor)'`) and `cap[target] = chars_at_preflight[target] + headroom[target]`. `cap[target]` is fixed for the whole run; § 3.4 sub-step (a.5) compares against a **fresh** re-measurement of the target file each time, not an incrementally-tracked running total (see (a.5) for why)
...
- **(a.5) Char-budget check** — before drafting (b)'s `Edit` call, re-measure the target file with `wc -m` to get `current_chars[target]` — a **fresh** read of the file's actual current state, not an incrementally-tracked running total. This is deliberate: a prior same-target Finding may have left uncommitted residue in the working tree (the (d) `conflict` branch does not revert (b)'s edit — see (d)'s `conflict` semantics), so only a fresh measurement is guaranteed to reflect what is actually on disk right now. Then estimate this Finding's naive char delta: `Σ (chars in new_string) − (chars in old_string)` across the fix's planned edits — count the planned `new_string` / `old_string` text directly (they are still in-memory `Edit` arguments, not yet on disk, so no `wc` invocation applies here; count in the same character unit `wc -m` uses). Compare `current_chars[target] + delta` against `cap[target]` (from § 1 Pre-flight's Char-budget cache).
  ...
  - **Over cap** — where the fix admits a **consolidation-form** redraft (folding the new content into an existing nearby reminder / paragraph via cross-reference instead of adding freestanding prose), redraft and re-estimate:
    ...
    - **Still over cap** — downgrade to `conflict` and continue: skip (b)–(g) for this Finding, set `record.disposition = conflict` (`record.target` stays at its default `—`, since (b) — the only sub-step that sets it — never runs), record warning `skill-md-growth budget exceeded (<projected> > cap <cap>) — deferred`, and mark the Finding for `budget-deferred` label application at § 3.5 Post triage comment.
...
- `skill-md-growth: <target>: +<N> chars (cap <cap>)` — one line per target with ≥1 committed Finding this run. Source of truth: a fresh `wc -m` re-measurement of the target file against `chars_at_preflight[target]` (the Pre-flight cache), taken at Step 4. Omit the line for targets with no commits this run
