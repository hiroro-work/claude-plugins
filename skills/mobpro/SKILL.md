...
**(2) User gates (closed list — approval / judgment).** M3 plan-building checkpoints (M3 sub-step 2.5's **Plan-building checkpoints** gate); M5 plan approval; M8 check/test fail-stop (error-stop after 3 retries) and the two scope stops; M9 persistent-violations gate (not under `--fast` — § Fast mode) and unresolved-findings gate (the latter only when `code_review_enabled`); M11 commit gates (stashing-hook incompatibility / commit-plan approval / per-commit accept — minus the commits the already-reviewed skip option covers / fold-or-defer / ambiguous-adjust); M12 gates (confirm-remaining-steps / rule-update commit / workability disposition); M13 deferral-exclusion gate and PR-URL prompt.
...
| --- | --- | --- |
| M4 (Plan review) | M1 sub-step 3 sets `plan_review_enabled = false` | `M4 — Plan review skipped (fast mode)` — appended only when `plan_review_enabled` was `true` beforehand, a `plan_review: "rules-only"` resolution included |
