...
A guided development workflow with fixed phases in a fixed order. What changes between runs is decided once, by one table, before any work starts, so a junior engineer can say what happens next at any point.

## Phases

| # | Phase | Tool | Skipped when |
|---|---|---|---|
| 1 | Load Settings | Read | never |
| 2 | Task Decomposition | Write | proposal only on Moderate / Complex |
| 3 | Create Plan | Read, Write | never |
| 4 | Plan Review | `Skill(<reviewer>)`; rules-only in `normal`, full in `--deep` | Trivial, `--fast` |
| 5 | Plan Approval | **user gate**: browser viewer (`serve.mjs`), chat fallback; plan artifact | never |
| 6 | Implement | Edit; a snapshot per Build order step | never |
| 7 | Tidy | `Skill(simplify)`, fallback `Skill(tidy)` | Trivial, Simple |
| 8 | Polish Prose | `Skill(prose-polish)` | Trivial, Simple, `polish_prose: false`, `--fast` |
| 9 | Check / Test | Bash, `Skill(run-tests)`; launches the two reviews in the background | never |
| 10 | Rules Compliance Review | `Skill(rules-review)` | Trivial, Simple |
| 11 | Code Review | `Skill(<reviewer>)` | Trivial, `code_review: false` |
| 12 | Verify Fixes | Bash, `Skill(rules-review)` | no review fixes |
| 13 | Completion Hooks | `hooks.on_complete` | key unset |
| 14 | Interactive Commits | **user gates**, git; one commit per Build order step with review fixes absorbed; crit browser with `commit_review_gate: crit` | never |
| 15 | Update Rules | **user gate** (covers phases 15–18), `Skill(extract-rules)` | extraction: Trivial, Simple |
| 16 | PR Rule Extraction | **user gate**, `Skill(extract-rules)` | empty answer |
| 17 | Self-Retrospective | **user gate**, `gh api` | `self_retrospective.feedback` unset, or skipped at the phase 15 gate |
| 18 | Workability Retrospective | **user gate**, project tooling candidates | `workability_retrospective.enabled` not `true`, or skipped at the phase 15 gate |
| 19 | Completion | summary | never |
...
Implement records a snapshot after every Build order step (a commit object on `refs/dev-workflow/<slug>`, built in a private index so your staging area is untouched). Before Interactive Commits, the edits made afterwards by Tidy, Polish Prose, the reviews, and Verify Fixes are attributed by `git blame` to the step that last wrote each changed line and folded into that step's commit (`scripts/absorb/attribute.mjs` plus a non-interactive `rebase --autosquash` in a throwaway worktree). Each commit you approve is therefore one step's work in its reviewed form. Edits no step owns, such as a new test file added by a review, land in one trailing `review fixes` commit. A file the run never touched — something another task had already edited, or an untracked file already sitting there when the run began — enters no commit at all and stays in the tree as you left it.

Absorption also handles formatter output, so put `lefthook run pre-commit` in `check_commands` and formatting lands before review. A multi-step task always yields several commits, so on a project with a pre-commit hook the stashing-hook question (suppress with `LEFTHOOK=0`, or proceed) appears on most runs.
...
Moderate and Complex tasks may be split into subtasks, each shipped as its own PR. The state file is `.claude/plans/dev-workflow.<slug>.md`, resumed with `--resume <slug>`.
