...
**Log and keys.** The How this page was built card is a `<details>` element, closed by default. Append the entry THIS TURN gives you, verbatim, as its last entry. When a message rewrites a first-line key, rewrite only that key in the first-line comment.
