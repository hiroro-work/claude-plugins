...
**(2) User gates (closed list — approval / judgment).** M3 plan-building checkpoints (M3 sub-step 2.5's **Plan-building checkpoints** gate); M5 plan approval and, when `plan_artifact` is `review`, the M5 team-review gate that follows it (`../dev-workflow/references/plan-artifact.md` § Team-review gate); M8 check/test fail-stop (error-stop after 3 retries) and the two scope stops; M9 persistent-violations gate (not under the `fast` run mode — § Run modes — and not on the express lane, whose skip of M9's rules half takes the gate with it) and unresolved-findings gate (the latter only when `code_review_enabled`); M11 commit gates (stashing-hook incompatibility / commit-plan approval / per-commit accept — minus the commits the already-reviewed skip option covers / fold-or-defer / ambiguous-adjust); M12 gates (confirm-remaining-steps — not fired when the express lane has left it no sub-phase to offer, per M12 sub-step 1 / rule-update commit, only where the rule-update sub-phase runs, so not on the express lane / workability disposition); M13 deferral-exclusion gate and PR-URL prompt.

Every stop in list (1) fires at every tier without exception, and so does every gate in list (2) but the ones annotated above: M9's persistent-violations gate, M9's unresolved-findings gate, M12's rule-update commit gate, and M12's confirm-remaining-steps gate on a run where the express lane leaves it no sub-phase to offer. A change that would let the tier or the run mode reach a further entry is checked against this paragraph before it lands anywhere else.
...
1. **confirm-remaining-steps entry gate** (presented whenever this run has at least one such sub-phase): ask whether to run the remaining rule-maintenance / retrospective work — listing only the sub-phases this run enabled and naming each by what it does, since M12's sub-phases are not registered as rows of their own, so take § Phase naming in user-facing output's **drop the number** option — or skip to M13 (Wrap-up). **When that list comes out empty** — the express lane has dropped the rule update and neither retrospective is configured — the gate does not fire: say in one line that nothing remains and go to M13 (Wrap-up). On skip, mark those `completed` without running and note the skip.
