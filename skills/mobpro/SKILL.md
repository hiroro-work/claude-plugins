...
**Entry condition — skipped entirely when `plan_review_enabled` is `false`**. Two causes, and no others — `mobpro` has no difficulty assessment, so nothing else lowers the flag: a configured `plan_review: false`, or `--fast` forcing it off at M1 sub-step 3 (§ Fast mode). M1 sub-step 4's registration burst already registered this step's row `completed`, so do not re-mark it: go straight to M5, whose presentation carries no "open points" (sub-step 4's open-points hand-off below never ran to produce any). The plan then reaches the junior unreviewed by the reviewer; M5's approval gate is its only review.
