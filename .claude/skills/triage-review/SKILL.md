...
(b) Invoke `Skill(skill-review)` with the short form (same shape as § Step 5 (b)'s `publicity-review` dispatch):

```
Base ref: main
```

`skill-review` accepts the `Base ref` field in its own `## Invocation contract`, so this form is contract-sanctioned. Do **not** append: triage branch name, the `changed_files` list, an explicit `git diff main HEAD` reference, or any other expansion beyond that one contract field.
...
`publicity-review` accepts the `Base ref` field in its own `## Invocation contract`, so this form is contract-sanctioned. `Max iterations` is left at its default.
