...
- feat(prose-polish): judge a target-language word by whether it decodes without the original
  - The litmus test exempted any token already written in the target language's own script, treating every katakana form as settled loanword prose — `セマンティクス` and `タイブレーク` passed through untouched. The exemption is gone. Those tokens are candidates like any other now, judged by one question: can a reader who does not know the source-language original recover the meaning from this word alone? `キャッシュ` and `レスポンス` still pass.
  - A second failure mode gets its own rule. A word-for-word rendering of an English figure of speech reads as the target language but only decodes back through the English (`着地する` for `land`, `〜に倒す` for `fall back to`, `走行` for a `run`, `閉じたリスト` for a `closed list`). General rule 6 now says to name what the thing does instead.
...
- feat(dev-workflow): polish the decomposition state file alongside the plan document
  - Step 4's plan-body polish took the plan document alone, so a decomposed run's subtask `description` / `verification_hint` prose — written for the user to read — was polished by no pass at all. Both paths now go to `prose-polish` in one invocation when a state file is active. That prose sits in YAML block scalars, so the dispatch tells the callee to leave leading indentation exactly as found.
- feat(dev-workflow): rule out sentence shapes that stay hard to read after the words are right
  - `localization.md` governed vocabulary only. Four sub-rules now govern construction: one claim per sentence and per bullet, references at the end rather than the front, one parenthetical per sentence, and no word that only decodes back through the source language. They carry the most weight in chat output, which no later polish pass can reach.
