...
**Ordering invariant — runs before § Auto-stash restore.** This section executes after Step 5 but **before § Auto-stash restore** (which runs at the top of § Step 6), so the working tree still equals HEAD here and `rules-review`'s `git diff main` equals `main..HEAD`.
...
1. `Skill(rules-review)` could not be resolved / dispatched in this environment (skill-not-found, or the call returned no usable response at all) → record `rules_review_result = {status: "skipped", reason: "unavailable"}`
...
**Run § Auto-stash restore before rendering any summary form** (it is a no-op when `auto_stashed == false`). Then append the `auto-stash:` field / warning line(s) defined in § Form 3 content:
...
- the **orphan warning** (`orphan_stash_detected == true`) attaches to **whatever form is emitted, including form 1**. (Form-1 aborts from checks 1–2 run before the orphan scan, so `orphan_stash_detected` is still `false` there and no orphan line is appended.)
...
- `auto-stash restore failed: <reason> — changes preserved in \`git stash list\`; resolve the conflict before the handoff \`git switch main\` step` — single line, **form 3 only**. On the form 2 path, emit the generic variant `auto-stash restore failed: <reason> — changes preserved in \`git stash list\`` instead
