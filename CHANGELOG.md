...
- feat(dev-workflow, mobpro): run configured commands as each implementation step's boundary is recorded
  - **Default: disabled** — set `boundary_check_commands` in `.claude/dev-workflow.md` to opt in per project. The new list key names shell commands that run at the moment a Build order step's boundary object is built, while that step's paths are staged; whatever they rewrite is re-staged and carried into that step's own tree. The motivating case is a project's pre-commit hook runner, which until now first ran at the commit step, long after the implementation it would have corrected. Placement is inside the boundary build rather than beside it, because a command that reads the staged set finds nothing before the `git add` and lands nothing in the tree after the `write-tree`. `dev-workflow` reads the key only where boundary objects exist, so `interactive_commits: false` leaves it unread; `mobpro` builds a per-unit object on every run and so always runs them, which also means the diff its per-unit review shows is the content that will be committed. A non-zero exit is recorded as a one-line note and the command is re-run once, then the boundary is built from whatever the working tree holds — `check_commands` at Step 7 stays the gate that stops the run. Commands outside the skills' `Bash(...)` grants may need a one-time permission approval. This narrows but does not remove the commit step's own hook handling: the final group has no boundary, and files a quality gate touches after their boundary are still unformatted when they land.
