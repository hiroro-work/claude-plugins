...
| `crit-commit-review.md` | M6 / M11 (only when `commit_review_gate` resolves to `crit`) | Diff review via crit — CLI contract, availability determination, story prologue, launch. M11 also takes its per-round unstage and per-commit outcome mapping; M6 defers unstaging to M6 exit and substitutes its own mapping (`references/diff-review.md` § crit path) |
...
4. Register all phases with the Task tools (`TodoWrite` fallback where Task tools are unavailable) in one upfront burst: the M1–M13 rows + `M4-1 … M4-N_plan` + `M9-1 … M9-N_code`. Subject each row per § Phase naming in user-facing output (`M6 — Implementation loop`; an iteration row appends its number — `M4-1 — Plan review iteration 1`), keeping the identifier so later instructions can resolve rows by subject. Conditional omissions (exactly three): omit `M2` in Resume sub-mode (the row only — M2's Resume path still runs, it just has nothing to register there); omit `M10` when `hooks.on_complete` is unset; omit `M11` when `interactive_commits` is false. `M12` is always one row (it branches internally on the 11.5 / 11.6 conditions).
